...
# Load 2: Load HCG model
# Load 3: Load Global model
# Predictions of Global model, HCG model, later fusion, and decision fusion
#----------------------
...
del model_haf, test_dataset, test_loader
#----------------------------------------------------------Load 2: Load HCG module---------------------------------------------------------------------
# dataset
...
import build_dataset
# modify
test_dataset_hcg = build_dataset.dataset_for_hcg(label_test, feature_v_test, word_predicts_test, decision_classes_topk_test, decision_words_test)
test_loader_hcg = torch.utils.data.DataLoader(test_dataset_hcg, batch_size=100, shuffle=False, **kwargs)

# load HCG
opt.method='hcg'
model_hcg = build_model.build(CUDA,opt)
# modify model name
model_hcg = build_model.get_updateModel(model_hcg, model_save_path + 'model_hcg.pt')
...
#--------------------------------------Predictions of Global model, HCG, later fusion, and decision fusion ----------------------------------------
# dataset
test_loader_hcg = torch.utils.data.DataLoader(test_dataset_hcg, batch_size=100, shuffle=False, **kwargs)
test_loader_global = torch.utils.data.DataLoader(test_dataset_global, batch_size=100, shuffle=False, **kwargs)
feature_global, output_global, label_global = test_functions.generate_feature_global(test_loader_global, model_global, opt)
feature_hcg, output_hcg, label_hcg = test_functions.generate_feature_hcg(test_loader_hcg, model_hcg, opt)

test_dataset_fusion = build_dataset.dataset_for_fusion(feature_global, feature_hcg, label_global)
...
top_k_pre = 5
label = label_global
pre_hcg = torch.topk(output_hcg, top_k_pre)[1]
pre_global = torch.topk(output_global, top_k_pre)[1]
# modify
pre_haf = decision_classes_topk_test[:,:top_k_pre]
...
for i in range(label.shape[0]):
    ...
    for k in range(top_k_pre):
        if k==0:
            print('Top-{} Class prediction: Global model: [{}], HAF: [{}], HCG: [{}] | final prediction: [{}]'.format(k+1, pre_global[i][k], pre_haf[i][k].long(), pre_hcg[i][k], pre_refine[i][k]))
        else:
            print('Top-{} Class prediction: Global model: [{}], HAF: [{}], HCG: [{}]'.format(k+1, pre_global[i][k], pre_haf[i][k].long(), pre_hcg[i][k]))