...
class FeatureFusion(nn.Module):
    ...
    def forward(self, feature_global, feature_hcg):
        feature_global = self.linearg2g(feature_global)
        feature_hcg = self.linearl2g(feature_hcg)
        feature_fusion = ((1.-self.beta) * feature_global) + (feature_hcg * self.beta)
...
def build(CUDA, opt):
    ...
    if opt.method=='global':
        ...
        model = HCG(dim_feat_v, opt.num_cls, opt.num_words, opt.topk, opt.beta_know, opt.beta_relation)
