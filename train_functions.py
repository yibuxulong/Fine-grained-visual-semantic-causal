...
def train_hcg(epoch, data_loader, model, optimizer, opt):
    acc1_v = AverageMeter('Acc@1', ':6.2f')
    acc5_v = AverageMeter('Acc@5', ':6.2f')
    losses = AverageMeter('Loss', ':.4e')
...
def generate_feature_hcg(data_loader, model):
    model.eval()
    with torch.no_grad():
        for batch_idx, (data, label) in enumerate(data_loader):
            [feature_v, word_predicts, decision_classes_topk, decision_words] = data                          
            ...
            feature_hcg, _ = model(word_predicts, feature_v, decision_words)
            if batch_idx==0:
    ...
    return feature_hcg_all.detach().cpu(), label_all.detach().cpu()
